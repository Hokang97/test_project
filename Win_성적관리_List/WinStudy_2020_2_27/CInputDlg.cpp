...
void CInputDlg::On_Initdialog(HWND hDlg)
{
	...
	g_MWTopLeft.x = 0; g_MWTopLeft.y = 0;
	...
	//SetWindowPos : 바탕화면 좌표 기준으로 움직인다.    
	//------------ 부모 윈도우 기준으로 적당한 위치(오른쪽상단)에 가서 달라 붙도록 설정하는 부분
...
void CInputDlg::On_IDOK(HWND hDlg)
{
	...
	if (a_NameStr[0] != '\0' &&
		a_KorStr[0] != '\0' && a_EngStr[0] != '\0' && a_MathStr[0] != '\0' &&
		a_KorStr != _T("") && a_EngStr != _T("") && a_MathStr != _T(""))
	{
			...
			0 <= a_Math && a_Math <= 100)
		{
			...
			else if (g_BtnState == EDIT_ST)
			{
				if (0 <= m_SelIndex)
				{
					...
					g_StList[m_SelIndex].m_Total = a_Kor + a_Eng + a_Math;
