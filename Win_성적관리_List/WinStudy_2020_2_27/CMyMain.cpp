...
void CMyMain::MainInit(HINSTANCE hInst, HWND a_hWnd)
{
	...
	HWND a_RefHwnd = CreateWindow(_T("button"), _T("입력"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
		10, 10, 100, 30, a_hWnd, (HMENU)IDC_INPUTSHOWBTN, hInst, NULL);
	m_WinList.push_back(a_RefHwnd);

	a_RefHwnd = CreateWindow(_T("button"), _T("총점순"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
		150, 10, 100, 30, a_hWnd, (HMENU)IDC_SORTTEXT, hInst, NULL);
	m_WinList.push_back(a_RefHwnd);

	a_RefHwnd = CreateWindow(_T("button"), _T("삭제"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
		290, 10, 100, 30, a_hWnd, (HMENU)IDC_DELETETEXT, hInst, NULL);
	m_WinList.push_back(a_RefHwnd);

	a_RefHwnd = CreateWindow(_T("button"), _T("선택목록수정"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
		450, 10, 100, 30, a_hWnd, (HMENU)IDC_EDITTEXT, hInst, NULL);
	m_WinList.push_back(a_RefHwnd);

	//--------------- 컬럼이 있는 리스트 컨트롤 만들기
	//////컬럼이 있는 리스트 컨트롤 만들기  #include <CommCtrl.h> 필요
	////확장스타일 적용  
	// | LVS_SHOWSELALWAYS  <-- 포커스가 비활성화 되더라도 선택상태가 보이도록 하는 옵션   
	// | LVS_SINGLESEL <-- 싱글 선택 옵션 디폴트는 멀티선택
	m_hListView = CreateWindow(WC_LISTVIEW, NULL, WS_CHILD | WS_VISIBLE | WS_BORDER
					| LVS_REPORT | LVS_SHOWSELALWAYS | LVS_SINGLESEL,
					20, 100, 540, 400, g_hWnd, (HMENU)IDC_LISTVIEW, hInst, NULL);
	////////확장스타일 적용
	...
	COL.iSubItem = 5;
	SendMessage(m_hListView, LVM_INSERTCOLUMN, 5, (LPARAM)&COL);  //여섯 번째 헤더 (WPARAM)4 컬럼 인덱스 --> 이 값이 작을 수록 앞에서부터 나온다. 클수록 뒤쪽으로 나온다.
	//--------------- 컬럼이 있는 리스트 컨트롤 만들기
...
{
	//TCHAR a_TempStr[256];
	//for (int ii = 0; ii < g_StList.size(); ii++)
	//{
	//	_stprintf_s(a_TempStr, _T("이름(%s) : 국어(%d) 영어(%d) 수학(%d) 총점(%d) 평균(%.1f)"),
	//		g_StList[ii].m_Name, g_StList[ii].m_Kor,
	//		g_StList[ii].m_Eng, g_StList[ii].m_Math,
	//		g_StList[ii].m_Total, g_StList[ii].m_Avg);

	//	TextOut(a_MainDC, 600, 80 + (ii * 30), a_TempStr, _tcslen(a_TempStr));
	//}

	if (0 <= g_SelChangeIdx && g_SelChangeIdx < g_StList.size())
	{
		static TCHAR a_Temp[256];
		_stprintf_s(a_Temp, _T("%s : 국어(%d) 영어(%d) 수학(%d) 총점(%d) 평균(%.2f)"),
			g_StList[g_SelChangeIdx].m_Name,
			g_StList[g_SelChangeIdx].m_Kor,
			g_StList[g_SelChangeIdx].m_Eng,
			g_StList[g_SelChangeIdx].m_Math,
			g_StList[g_SelChangeIdx].m_Total,
			g_StList[g_SelChangeIdx].m_Avg);
...
void CMyMain::MainDestroy()
{
	...
	{
		for (int ii = 0; ii < (int)m_WinList.size(); ii++)
		{
	...
	switch (message)
	{
		case WM_COMMAND:
		{
			...
			switch (wmId)
			{
				...
				{
					g_BtnState = INPUT_ST;
					g_SelChangeIdx = -1;  //선택 상태 정보 표시를 초기화를 위하여...
					InvalidateRect(g_hWnd, NULL, TRUE);  //선택 상태 정보 표시를 초기화를 위하여...

					DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG1), g_hWnd,
						(DLGPROC)CInputDlg::InputDlgProc);
				}
				break;

				case IDC_SORTTEXT:         //총점순으로 정렬하기 버튼
				{
				...
				case IDC_EDITTEXT:         //선택 목록 수정 버튼
				{
					g_BtnState = EDIT_ST;
					DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG1), g_hWnd,
										(DLGPROC)CInputDlg::InputDlgProc); //다이알로그박스
				}
				break;
